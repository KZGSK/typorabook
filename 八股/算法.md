...
class LRUCache {
    ...
    private void addRecently(int key, int val, long expireTime) {
        Node node = new Node(key, val, System.currentTimeMillis() + expireTime);
        cache.addLast(node);
        map.put(key, node);
    }
    ...
    public void put(int key, int value) {
        if (map.containsKey(key)) {
            Node node = map.get(key);
            node.val = value;
            node.expireTime = System.currentTimeMillis() + defaultExpireTime;
            makeRecently(key);
            return;
        }
        if (cap == cache.size()) {
        ...
        addRecently(key, value, defaultExpireTime);
    }

    // Optional: Method to clean up expired nodes periodically
